...
把 daemon 注册到当前用户的 init 系统（macOS 用 launchd，Linux 用 user systemd），**不需要 sudo**。重启机器、退出再登录都不用再手动 `botmux start`。

- 这条命令只管"开机时自动起"，不会动你**当前**正在跑的 daemon。
- Linux 服务器登出会话后想让服务继续跑：另跑一次 `sudo loginctl enable-linger <你的用户名>`，命令本身会在 linger 没开时主动提醒。
- 想关掉自启：`botmux autostart disable`（同样不会停 daemon）；查状态：`botmux autostart status`。
- 切换 nvm/fnm Node 版本后跑一次 `botmux autostart enable` 重写路径即可——`botmux start`/`restart` 也会自动检测路径漂移。

详见下方 [CLI 命令 § 开机自启](#开机自启)。
