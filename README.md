...
**飞书话题群 + AI 编程 CLI，一条消息启动编程会话。** Daemon 监听飞书消息，为每个新话题自动启动独立 CLI 进程（Claude Code / Codex / Cursor / Gemini / OpenCode / Antigravity / Pi / Pi RPC），提供实时流式卡片和可交互 Web 终端。
...
botmux 不重新实现 Agent 能力，而是直接桥接已有的 AI 编程 CLI（Claude Code、Codex、Cursor、Gemini、OpenCode、Antigravity、Pi、Pi RPC）。记忆、上下文管理、工具调用、权限体系——这些能力 CLI 本身都在快速迭代，botmux 选择站在这个进化之上，而不是平行重造一套。CLI 的每次升级，botmux 零适配自动受益。
...
| 多 CLI 支持 | 8 种 CLI 一键切换（Claude Code / Codex / Cursor / Gemini / OpenCode / Antigravity / Pi / Pi RPC） | 绑定单一 SDK，无法切换 CLI |
...
这些能力通过 `--append-system-prompt` 注入和 Skill 描述自动引导 agent 使用。Skill + CLI 的组合相比 Anthropic 官方 Telegram channel 那套 MCP 方案：CLI 启动不用做 MCP 握手、不占用工具列表 token，且对 Claude Code / Codex / Cursor / Gemini / OpenCode / Antigravity / Pi / Pi RPC 通用 —— 只要 CLI 能读 system prompt 跑 shell 命令就行，不依赖任何 MCP 协议支持。
...
- **Node.js** >= 20
- **AI 编程 CLI** 已安装并完成认证（`claude`、`codex`、`cursor-agent`、`gemini`、`opencode`、`mtr`、`agy`（Antigravity）或 `pi` 在 PATH 中（`pi-rpc` 复用同一个 `pi` 二进制））
...
| `cliId` | 否 | CLI 适配器，默认 `claude-code`（可选：`aiden`、`coco`、`codex`、`cursor`、`gemini`、`opencode`、`antigravity`、`mtr`、`pi`、`pi-rpc`） |
