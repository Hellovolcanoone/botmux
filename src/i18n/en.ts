...
export const messages: Record<string, string> = {
  ...
  'cmd.config.unknown_sub': 'Unknown subcommand: {sub}\nSupported: /botconfig get | set | unset | targets | relay | trust | help',
  'cmd.config.humans_only': '⛔ Editing allowedUsers / trust is human-only — it cannot be relayed through a controller bot (anti-escalation).',
  'cmd.config.relay_need_group': 'This subcommand must be used in a group chat (it reads other bots in the group).',
  'cmd.config.targets_none': 'No other relayable bots found in this group. Add the target bot to this group and have it @ this bot once (or /introduce each other), then retry.',
  'cmd.config.targets_list': '🛰️ Relayable targets in this group:\n{list}\n\nRelay: /botconfig relay <name> <subcommand>, e.g. /botconfig relay <name> set model opus',
  'cmd.config.target_unreachable': '(not @-mentionable yet — @ each other once to establish the mapping)',
  'cmd.config.relay_usage': 'Usage: /botconfig relay <target name or ou_id> <subcommand>\ne.g.: /botconfig relay codexbot set model opus　/botconfig relay codexbot get',
  'cmd.config.relay_not_found': 'Target "{query}" not found. Relayable targets here: {candidates} (see /botconfig targets)',
  'cmd.config.relay_unreachable': 'Target "{name}" is not @-mentionable yet (cross-app open_id mapping missing). Have it @ this bot in the group, or /introduce each other, then retry.',
  'cmd.config.relay_send_failed': '⚠️ Relay failed: {msg}',
  'cmd.config.relay_sent': '📤 Relayed to "{name}": {cmd}\nThe target replies with the result in this group (it must have /botconfig trust-ed this bot).',
  'cmd.config.trust_list': '🔑 Current allowedUsers (incl. trusted controller bots):\n{list}',
  'cmd.config.trust_usage': 'Usage: /botconfig trust <target name> to authorize a bot as this bot\'s controller; /botconfig trust list to view; /botconfig untrust <name> to revoke.',
  'cmd.config.trust_already': '"{name}" is already in allowedUsers — no need to re-authorize.',
  'cmd.config.trust_ok': '✅ Authorized "{name}" as a controller (open_id {id} added to allowedUsers). Effective immediately — it can now relay /botconfig to this bot.',
  'cmd.config.untrust_absent': '"{name}" is not in allowedUsers — nothing to revoke.',
  'cmd.config.untrust_ok': '✅ Revoked "{name}"\'s control (removed from allowedUsers). Effective immediately.',
