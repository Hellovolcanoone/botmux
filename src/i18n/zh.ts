...
export const messages: Record<string, string> = {
  ...
  'cmd.config.help': '⚙️ /botconfig —— 远程改本机器人运营配置（仅 owner/allowedUsers，写盘即热更新，无需重启）\n\n用法：\n/botconfig get              — 查看当前配置\n/botconfig set <字段> <值>  — 修改\n/botconfig unset <字段>     — 清回默认（部分字段支持）\n\n跨机器（控制 bot 在共享群里下发给其它机器的 bot）：\n/botconfig targets          — 列出本群里可下发的目标 bot\n/botconfig relay <名称> <子命令>  — 让本 bot @ 目标 bot 下发 /botconfig（如 relay codexbot set model opus）\n/botconfig trust <名称>     — 授权某 bot 作为本 bot 的「控制者」（加进 allowedUsers）；/botconfig untrust 撤销\n\n可改字段：\n{fields}\n\n敏感字段 allowedUsers 需在结尾加 `确认` 才生效；secret（appSecret/凭证）不能经此修改。',
  ...
  'cmd.config.unknown_sub': '未知子命令：{sub}\n支持：/botconfig get | set | unset | targets | relay | trust | help',
  'cmd.config.humans_only': '⛔ 改 allowedUsers / 授信只能由真人操作，不能经控制 bot 中转（防提权）。',
  'cmd.config.relay_need_group': '该子命令需在群聊里使用（要读取群内其它 bot）。',
  'cmd.config.targets_none': '本群里没有发现其它可下发的 bot。把目标 bot 拉进本群、并让它 @ 一下本 bot（或互相 /introduce）后再试。',
  'cmd.config.targets_list': '🛰️ 本群可下发目标：\n{list}\n\n下发：/botconfig relay <名称> <子命令>，例：/botconfig relay <名称> set model opus',
  'cmd.config.target_unreachable': '（暂不可 @，需先互相 @ 一次建立映射）',
  'cmd.config.relay_usage': '用法：/botconfig relay <目标名称或 ou_id> <子命令>\n例：/botconfig relay codexbot set model opus　/botconfig relay codexbot get',
  'cmd.config.relay_not_found': '未找到目标「{query}」。本群可下发目标：{candidates}（用 /botconfig targets 查看）',
  'cmd.config.relay_unreachable': '目标「{name}」暂时 @ 不到（open_id 跨应用未建立映射）。先让它在本群 @ 一下本 bot，或互相 /introduce 后再试。',
  'cmd.config.relay_send_failed': '⚠️ 下发失败：{msg}',
  'cmd.config.relay_sent': '📤 已向「{name}」下发：{cmd}\n执行结果由对方在本群回复（需对方已 /botconfig trust 本 bot）。',
  'cmd.config.trust_list': '🔑 当前 allowedUsers（含被授信的控制 bot）：\n{list}',
  'cmd.config.trust_usage': '用法：/botconfig trust <目标名称>　授权某 bot 作为本 bot 的控制者；/botconfig trust list 查看；/botconfig untrust <名称> 撤销。',
  'cmd.config.trust_already': '「{name}」已在 allowedUsers 中，无需重复授权。',
  'cmd.config.trust_ok': '✅ 已授权「{name}」作为控制者（open_id {id} 加入 allowedUsers），立即生效。此后它可经 /botconfig relay 改本 bot 配置。',
  'cmd.config.untrust_absent': '「{name}」不在 allowedUsers 中，无需撤销。',
  'cmd.config.untrust_ok': '✅ 已撤销「{name}」的控制权（移出 allowedUsers），立即生效。',
